...
import { TextEditorTab } from './components/tabs/TextEditorTab';
...
export const App: React.FC = () => {
  ...
  // Tab State
  const [currentTab, setCurrentTab] = useState<'text' | 'remix' | 'audio' | 'video'>('text');
  ...
  return (
      <div
          className="h-screen bg-transparent text-text-main font-sans selection:bg-primary/30 selection:text-white flex flex-col overflow-hidden">
          ...
          {isExporting && (
              <div
                  className="fixed inset-0 z-[100] bg-black/80 flex flex-col items-center justify-center gap-6 backdrop-blur-xl animate-fade-in">
                  ...
                  <div
                      className="w-96 h-1.5 bg-white/10 rounded-full overflow-hidden border border-white/5">
                      <div
          ...
          {/* TOP HEADER */}
          <header
              className="h-14 px-6 glass-panel border-b border-border flex items-center justify-between shrink-0 z-50">
              <div className="flex items-center gap-3">
                  ...
                  <div className="flex flex-col">
                      <h1
                          className="text-sm font-bold bg-clip-text text-transparent bg-gradient-to-r from-text-main to-text-muted tracking-tight">
              ...
              {/* TAB NAVIGATION */}
              <div
                  className="flex items-center bg-surface/50 rounded-lg p-1 border border-border">
                <button
                    onClick={() => setCurrentTab('text')}
                    className={`px-4 py-1.5 rounded-md text-xs font-medium transition-all ${currentTab === 'text' ? 'bg-primary text-white shadow-sm' : 'text-text-muted hover:text-text-main hover:bg-surface'}`}>
                  1. Text Editor
                </button>
                <div className="w-px h-4 bg-border mx-1"></div>
                <button
                    onClick={() => {
                      if (state.beats.length === 0) handleAnalyzeBeats();
                      setCurrentTab('remix');
                    }}
                    className={`px-4 py-1.5 rounded-md text-xs font-medium transition-all ${currentTab === 'remix' ? 'bg-primary text-white shadow-sm' : 'text-text-muted hover:text-text-main hover:bg-surface'}`}>
                  2. Remix
                </button>
                <div className="w-px h-4 bg-border mx-1"></div>
                <button
                    onClick={() => setCurrentTab('audio')}
                    className={`px-4 py-1.5 rounded-md text-xs font-medium transition-all ${currentTab === 'audio' ? 'bg-primary text-white shadow-sm' : 'text-text-muted hover:text-text-main hover:bg-surface'}`}>
                  3. Audio
                </button>
                <div className="w-px h-4 bg-border mx-1"></div>
                <button
                    onClick={() => setCurrentTab('video')}
                    className={`px-4 py-1.5 rounded-md text-xs font-medium transition-all ${currentTab === 'video' ? 'bg-primary text-white shadow-sm' : 'text-text-muted hover:text-text-main hover:bg-surface'}`}>
                  4. Video Editor
                </button>
              </div>
              ...
              <div className="flex items-center gap-4">
                   {/* Status Pill */}
                   <div
                       className={`flex items-center gap-2 px-3 py-1.5 rounded-full border transition-all duration-300 ${status ? 'bg-primary/5 border-primary/20 opacity-100' : 'opacity-0 border-transparent'}`}>
                       {status && <span
                           className="w-1.5 h-1.5 rounded-full bg-primary animate-pulse shadow-[0_0_8px_rgba(99,102,241,0.6)]"></span>}
                       <span className="text-[10px] font-medium text-primary tracking-wide">{status}</span>
                   </div>
                   
                   {error && (
                     <div
                         className="flex items-center gap-2 px-3 py-1.5 bg-red-500/10 border border-red-500/20 rounded-full text-red-400 text-[10px] font-medium animate-fade-in">
                        <AlertCircle size={12} />
                        {error}
                     </div>
                   )}
          ...
          {currentTab === 'text' && (
            <TextEditorTab
                ...
                setSelectedScriptText={setSelectedScriptText}
                ...
                selectedScriptText={selectedScriptText}
                handleAIInsert={handleAIInsert}
                handleAIReplace={handleAIReplace}
                ...
                className="border border-amber-300 bg-amber-500" />
          )}
          {currentTab === 'remix' && (
            <RemixTab
                scriptBlocks={state.scriptBlocks}
                beats={state.beats}
                onNext={() => setCurrentTab('audio')}
                isAnalyzing={isAnalyzing} />
          )}
          {currentTab === 'audio' && (
            <AudioTab
                beats={state.beats}
                onNext={() => setCurrentTab('video')}
                handleGenerateVeo={handleGenerateVeo}
                handleUploadARoll={handleUploadARoll}
          ...
          {currentTab === 'video' && (
            <VideoEditorTab
                ...
                handleAddVideoComment={handleAddVideoComment}
                handleDeleteVideoComment={handleDeleteVideoComment}
                handleReplyVideoComment={handleReplyVideoComment}
                handleResolveVideoComment={handleResolveVideoComment}
                updateBeat={updateBeat}
                handleRegenerateImage={handleRegenerateImage}
                handleUploadImage={handleUploadImage}
                handleSplitBeat={handleSplitBeat}
                handleMergeBeats={handleMergeBeats}
                handleGenerateImageFromPrompt={handleGenerateImageFromPrompt}
                handleSelectImage={handleSelectImage}
                styleSettings={state.styleSettings}
                updateStyleSettings={updateStyleSettings}
                handleBulkGenerate={handleBulkGenerate}
                isAnalyzing={isAnalyzing}
                setLayoutMode={(mode) => setState(prev => ({ ...prev, layoutMode: mode }))} />
          )}
      </div>
  );
};
